...
than executed at site-build time — several depend on network access
(Wikipedia, Ollama), on data files that live outside this repo in
`../IF/data/`, or take a while to run (topic models). Run them yourself
in RStudio.
:::
...
it crucially shapes what people know (about the past). Already in
2010, a majority of adult US internet users used Wikipedia for
information retrieval (Heimlich 2011). As research has shown, what is
written on Wikipedia pages matters: it influences media content,
scientific research, and ordinary citizens' real-life decisions.
Journalistic work has additionally shown that political groups
proactively seek to edit Wikipedia in their favor. Previous research
has shown that the number of editors on Wikipedia is limited and
predominantly male, reflecting persisting patterns of gender inequality
...
clean_text_data <- qread("../../IF/data/wikipedia_nobel_biographies_summaries_clean.qs")
...
- Read the `wikipedia_nobel_biographies_summaries_clean_extended.qs`
...
We use the [quanteda](https://quanteda.io/) package for text
preprocessing.
...
Inverse Document Frequency (IDF = ln(N/df)): blue=0.693, car=0,
fast=0.693, red=0.693, slow=0.693
...
### Task: calculate a small DFM by hand
